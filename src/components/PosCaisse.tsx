...
export default function PosCaisse({ 
  ...
  const addToCart = (product: Product) => {
    ...
    if (product.isOutOfStock) {
      const msg = isRtl 
        ? 'السلعة غير متوفرة! نفدت هذه السلعة حالياً.' 
        : 'Rupture de stock ! Cet article est indisponible.';
      setPosWarning(msg);
      alert(msg);
      return;
    }
          ...
          {filteredProducts.map((p) => {
            ...
            return (
              ...
              >
                ...
                <div className="mt-3 pt-2.5 border-t border-gray-100 space-y-2">
                  ...
                  <div className="flex items-center justify-between text-[11px]">
                    ...
                    {p.isOutOfStock ? (
                      ...
                      <span className="px-2 py-0.5 bg-emerald-50 text-emerald-800 text-[9px] font-black uppercase rounded shadow-xs border border-emerald-100">
                ...
                {p.isOutOfStock && (
                  <div className={`absolute top-1.5 p-0.5 bg-rose-600 text-white rounded-full ${isRtl ? 'left-1.5' : 'right-1.5'} shadow`}>
