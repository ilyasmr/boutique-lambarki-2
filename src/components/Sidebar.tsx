      ...
      <div className="p-6 border-b border-slate-100 bg-slate-50/60 flex flex-col items-center gap-3 relative">
        ...
        <div className="flex items-center gap-2.5 mt-2 lg:mt-0 text-center justify-center">
          <div className="text-center">
            <h1 className="text-[24px] font-bold text-emerald-600 leading-none font-trad" style={{ fontFamily: 'Amiri, serif' }}>محل المباركي 2</h1>
            <p className="text-[11px] text-slate-500 font-semibold italic tracking-wide mt-1.5 lowercase">lambarki boutique 2</p>
          </div>
        </div>
      </div>
        ...
        {allowedMenuItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-sm transition-all duration-300 group cursor-pointer active:scale-95 ${
                isActive
                  ? 'bg-gradient-to-r from-emerald-500 to-teal-600 font-extrabold text-white shadow-[0_4px_14px_0_rgba(16,185,129,0.39)]'
                  : 'font-semibold text-slate-500 hover:text-slate-800 hover:bg-slate-50/80 hover:shadow-sm hover:scale-[1.01]'
              }`}
            >
              <Icon className={`w-5 h-5 transition-transform duration-300 group-hover:scale-110 ${isActive ? 'text-white' : 'text-slate-400 group-hover:text-emerald-500'}`} />
              <span className="truncate">{item.label}</span>
              {item.badge && item.badge > 0 && (
                <span className={`px-2 py-0.5 rounded-full text-[10px] font-black ${isRtl ? 'mr-auto' : 'ml-auto'} ${
