...
export default function ProductsList({ 
  ...
  // Helper to obtain a unique chronological index for each product
  const getSequentialNumber = React.useCallback((product: Product) => {
    const sortedById = [...products].sort((a, b) => a.id.localeCompare(b.id));
    const index = sortedById.findIndex(p => p.id === product.id);
  ...
  const [availabilityFilter, setAvailabilityFilter] = React.useState<'all' | 'available' | 'out_of_stock'>('all');
  ...
  const [formIsOutOfStock, setFormIsOutOfStock] = React.useState(false);
  ...
  const handleEditClick = (p: Product) => {
    ...
    setFormIsOutOfStock(!!p.isOutOfStock);
    setIsOpenModal(true);
  };
  ...
  const filteredProducts = React.useMemo(() => {
    return products.filter(p => {
      ...
      if (availabilityFilter === 'available') matchesAvailability = !p.isOutOfStock;
      if (availabilityFilter === 'out_of_stock') matchesAvailability = !!p.isOutOfStock;
    ...
    <div className="space-y-6" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm space-y-4">
        ...
        <div className="flex flex-col lg:flex-row gap-4 items-stretch lg:items-center justify-between">
          ...
          {/* Removed Low Stock Widget as requested */}
          ...
          <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
            ...
            <div className="flex items-center gap-1.5 bg-slate-50 border border-slate-200 px-3 py-2.5 rounded-xl text-xs font-bold shrink-0">
              ...
              <select
                ...
                className="bg-transparent text-slate-700 outline-none cursor-pointer text-xs font-extrabold border-none p-0 focus:ring-0"
              >
                <option value="all">{isRtl ? 'حالة التوفر: الكل' : 'Tous les Statuts'}</option>
                <option value="available">{isRtl ? 'المتوفرة فقط' : 'Disponibles'}</option>
                <option value="out_of_stock">{isRtl ? 'التي نفدت فقط' : 'En Rupture'}</option>
              </select>
            </div>
              ...
              >
                <div>
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-[9px] font-mono text-slate-400 font-bold">ID: {getSequentialNumber(p).toString().padStart(3, '0')}</span>
                  </div>
                ...
                <div className="mt-4 pt-3 border-t border-gray-100 space-y-3">
                  ...
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-slate-500 font-bold">{isRtl ? 'نفاد السلعة؟' : 'Indisponible?'}</span>
                ...
                {/* Out of Stock alerting indicator overlay */}
                {p.isOutOfStock && (
                  <div className={`absolute top-2.5 p-1 bg-rose-600 text-white rounded-full ${isRtl ? 'left-2.5' : 'right-2.5'} shadow`}>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-gray-50/60 border-b border-gray-100 text-xs font-bold uppercase text-gray-400">
                  ...
                  <th className="py-3 px-4 text-center">{isRtl ? 'نفاد السلعة؟' : 'Indisponible?'}</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {filteredProducts.map((p) => {
                  const isLowStock = p.stock <= p.minStockAlert;
                  return (
                    <tr key={p.id} className={`text-xs hover:bg-slate-50/80 transition-all duration-300 hover:shadow-sm hover:z-10 relative hover:scale-[1.002] ${p.isOutOfStock ? 'opacity-65 bg-slate-50/30' : ''}`}>
                      <td className="py-4 px-4 font-mono font-bold text-gray-500">
                        {getSequentialNumber(p).toString().padStart(3, '0')}
                      </td>
                      ...
                      <td className="py-4 px-4 text-center">
                        <label className="inline-flex items-center cursor-pointer justify-center">
                          <input 
                            ...
                            className="sr-only peer"
                          />
                          <div className="relative w-9 h-5 bg-gray-250 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-rose-500"></div>
                          <span className="ms-2 text-xxs font-bold text-slate-600">
                            {p.isOutOfStock ? (isRtl ? 'منفد' : 'Rupture') : (isRtl ? 'متوفر' : 'Disponible')}
                          </span>
                        </label>
                      </td>
                      <td className="py-4 px-4 text-center">
